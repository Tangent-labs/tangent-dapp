...
export function NeonMetricsCard({ title, subtitle, value, extra, color1, color2, className }: NeonMetricCardProps) {
  return (
    <div className={`relative ${className ?? ""}`}>
      ...
      <NeonLightCard paddingHorizontal={3} color1={color1} color2={color2} className="h-full w-full">
        <span className="mt-1 flex w-full items-center justify-center text-sm text-subtitle">{subtitle}</span>

        <div className="mb-2 flex items-center justify-center gap-1">
          <span className="text-sm font-semibold text-white">{value}</span>
          {extra ? <span className="bg-row-tonic bg-clip-text text-xs font-semibold text-transparent">{extra}</span> : null}
        </div>
      </NeonLightCard>
    </div>
  )
