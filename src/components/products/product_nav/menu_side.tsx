...
export function MenuSide({ initialIsOpen }: MenuSideProps) {
  const [isOpen, setIsOpen] = useCookieState<boolean>(dappConfig.keyPaths.navIsOpen, initialIsOpen)
  return (
    <nav className={cn(" flex justify-between items-center gap-2 relative ")}>
      <Panel className={cn("min-h-[98dvh] !p-0 transition-all duration-500  ease-in-out ", !isOpen ? "w-[70px]" : "w-[250px]")}>
        <Panel className="text-3xl !p-2 ">
          <div className="flex items-center">
            <Logo />
            <span className={cn("overflow-x-hidden", !isOpen && "hidden")}>Tangent</span>
          </div>
        </Panel>
        <div className={cn("px-2 mt-6 flex flex-col gap-4", isOpen ? "px-2" : "px-1")}>
          ...
          {/* Product TGUSD  ---------------------------------------- */}
          <div>
            <div className={cn("flex gap-5 items-center ml-4 text-gray-700  duration-200 overflow-x-clip", !isOpen ? "opacity-0 hidden" : " opacity-100")}>
              <div className="flex items-center justify-between w-full mr-2">
                <div className="flex items-center gap-2">
                  <Image width={40} height={40} src="/medias/product_tgusd.png" alt="tg USD" />
                  <span className="text-gray-700 font-semibold">tgUSD</span>
                </div>
                <span className=" bg-button-active text-black px-4 py-1 rounded-full">Soon</span>
              </div>
            </div>
          </div>
          {/* Docs  ---------------------------------------- */}
          <div>
            <a className={cn("flex gap-3 items-center  overflow-x-clip", isOpen && "ml-4")} href="#">
      ...
      <Panel
        className={cn("!w-[25px] h-36 !p-0  flex justify-center items-center cursor-pointer")}
        onClick={() => {
          setIsOpen(!isOpen)
        }}
      >
