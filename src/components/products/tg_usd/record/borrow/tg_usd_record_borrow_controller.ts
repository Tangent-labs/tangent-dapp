...
export function getFormState(marketData?: MarketDetailData, borrowWeiValue?: bigint, isWellConnected?: boolean) {
  const reasons: string[] = []
  // check the wallet
  if (!isWellConnected) {
    reasons.push("No connected wallet.")
  } else {
    const borrowReasons = getBorrowState(marketData, 0n, borrowWeiValue)
